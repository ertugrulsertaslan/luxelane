...
export default function(data,thumbnail) {
    ...
    const branchId = 1;
    db.run(sql,[data.carTypeId,data.model,data.hp,data.seats,data.hourlyPrice,thumbnail,data.fuel,data.transmission,data.bodyType,data.doors,data.minDriverAge,data.minLicenseAge,data.zeroToHundredKmh, data.status, branchId, data.brandId],(err, rows) => {
        if (err) {
            reject(err);
        }
        resolve(rows);
